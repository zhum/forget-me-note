...
Проект программы-органайзера "что где лежит" для android. Назначение программы - помочь вспомнить где лежит какая-то вещь (рубанок, зимние ботинки и т.п.). Чтобы внести в программу информацию о том, в каком ящике/столе/полке/шкафу какие вещи лежат, сфотографируйте их и отметьте вещи. Можно добавлять фото с текстом (подписью) или только текст. На фото при добавлении новой вещи необходимо отметить её рамкой. Можно (нужно!) добавлять иерархию - сфотографировали шкаф, затем добавили его полки (отметив их на фото шкафа), и т.д.
Для поиска нужно нажать копку поиск в интерфейсе и выбрать вещь из списка, в результате перейдём на фото места, где она лежит. Если место сходу непонятно по фото и названию, то идём вверх по иерархии, чтобы понять где это и т.д.
...
### 1. Navigation
  ...
  - Название элемента + кнопка его редактирования.
  - Кнопка настроек.
...
### 3. Delete (третий режим Navigation)
  ...
  Кнопка "режим" меняет режим на "Navigation".
...
### 4. Foto
  ...
  - назад
  - пропустить (фото не делается, сразу переход к следующему шагу)
...
### 5. Foto 2
  ...
  автоматически выравниваются по добавленным ранее рамкам. Это поведение
  отключается, галочкой "прилипать", состояние галочки глобальное и запоминается.
...
### 6. Search
  ...
  вверху в списке подсвечивается (и если надо - перематывается позиция
  ...
  **Замечание** предусмотреть поиск с помощью частичного ввода названия вещи в
...
При добавлении элемента можно ввести подпись вручную, а можно выбрать из выпадающего списка (и отредактировать потом).
Например, по умолчанию в списке будут "Комната 1", "Шкаф 1", "Полка 1".

Если пользователь выбирает "Комната 1", он может отредактировать это имя.
...
- клик по элементу = подсветка его в списке и показ его фото. Если фото
  нет, показываем фото родителя с подсветкой элемента. Если у родителя
